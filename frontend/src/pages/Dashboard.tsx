  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const [studentRes, projectsRes] = await Promise.all([
        fetch(`http://localhost:8082/api/auth/student/${student.id}`),
        fetch(`http://localhost:8082/api/projects/student/${student.id}`),
      ]);
      if (studentRes.ok) {
  ...
  const fetchDailyProgress = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:8082/api/tasks/students/${student.id}/daily-progress`
      );
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      if (newName && newName !== student.name) {
        await axios.put(
          `http://localhost:8082/api/auth/student/${student.id}/update-name`,
          { name: newName }
        );
      ...
      if (currentPassword && newPassword) {
        await axios.put(
          `http://localhost:8082/api/auth/student/${student.email}/change-password`,
